...
import com.jjh.framework.plugin.excel.Excel.Type;
...
public class ExcelUtil<T>
{
     ...
     * @author jjh
     */
    private void initDictConverter() {
        // 获取字典转换类
    ...
    public List<T> importExcel(MultipartFile file)
    {
        try {
            ...
            e.printStackTrace();
            throw new BusinessException("文件导入异常："+ e.getMessage());
        }
    }
        ...
        if (rows > 0)
        {
            ...
            for (int i = 1; i < rows; i++)
            {
                ...
                for (Map.Entry<Integer, Field> entry : fieldsMap.entrySet())
                {
                    ...
                    else if (StringUtils.isNotEmpty(attr.dictConverter())) {
                        val = sysDictDataService.getDictDataCode(attr.dictConverter(), String.valueOf(val));
                    }
                    ...
                    else if (Date.class == fieldType)
                    {
                        ...
                        {
                            if (StrUtil.isBlank(String.valueOf(val))) {
                                val = null;
                            }
                            else {
    ...
    public String exportExcel(List<T> list, String sheetName)
    {
        ...
        log.info("ExportExcel: {} Spend time: {}s", fileName, (System.currentTimeMillis() - startTime)/1000);
        return fileName;
    }
    ...
    public void setDataValidation(Excel attr, Row row, int column)
    {
        ...
        {
            // 如果设置了dictConverter属性则本列只能选择不能输入，填入选择项为字典值
            List<SysDictData> dictDataList = sysDictDataService.listByDictType(attr.dictConverter());
            if (CollectionUtil.isNotEmpty(dictDataList)) {
                List<String> dictDataNameList = new ArrayList<>();
                for (SysDictData dictData : dictDataList) {
                    dictDataNameList.add(dictData.getName());
                }
                String[] textList = new String[dictDataList.size()];
                dictDataNameList.toArray(textList);
                // 这里默认设了2-101列只能选择不能输入.
                setXSSFValidation(sheet, textList, 1, 100, column, column);
            }
        }
    }
    ...
    public Cell addCell(Excel attr, Row row, T vo, Field field, int column)
    {
        ...
        try
        {
            ...
            if (attr.isExport())
            {
                ...
                String dictConverter = attr.dictConverter();
                ...
                else if (StringUtils.isNotEmpty(dictConverter) && value != null)
                {
                    cell.setCellValue(sysDictDataService.getDictDataName(dictConverter, String.valueOf(value)));
                }
                else
                {
                    // 设置列类型
    ...
    /**
     * 获取导出值 字典：0=男,1=女,2=未知 得到 男/女/未知
     *
     * @param converterExp 翻译注解
     * @return 解析后值
     * @throws Exception
     */
    public static String[] convertByExp(String converterExp)
    {
        String[] dictValueArray = null;
        String[] convertSource = converterExp.split(",");
        dictValueArray = new String[convertSource.length];
        for (int i = 0; i < convertSource.length; i++) {
            String[] itemArray = convertSource[i].split("=");
            dictValueArray[i] = (itemArray[1]);
        }
